...
import DocExampleViewer from '@/components/doc/DocExampleViewer';
...
const components = {
    ...
    a: ({ className, ...props }: React.HTMLAttributes<HTMLAnchorElement>) => {
        ...
        return <a className={cn(isAnchorLink ? '' : 'font-medium text-(--primary-text-color) hover:!underline', className)} target="_blank" rel="noopener noreferrer" {...props} />;
    },
    ...
    code: ({ className, ...props }: React.HTMLAttributes<HTMLElement>) => {
        ...
        return <code className={cn(isInlineCode ? 'bg-(--mark-background) text-surface-900 dark:text-surface-0 rounded-md px-1.25 py-0.5 tracking-tight' : '', className)} {...props} />;
    }
};
