...
export default function DocDemoWrapper({
    ...
    const demo = name.startsWith('ui/') ? name : name.split(':')[1];
                    ...
                    <div className="p-1.5 flex items-center gap-2 border-b border-(--code-figure-border)">
                        ...
                        <div className="flex items-center justify-end flex-1">
                            <div className="flex items-center gap-px">
                                <CaptionButton className="" onClick={() => setIsExpanded((prev) => !prev)}>
                                ...
                                <DocCopyButton className="size-8! [&_i]:text-sm!" source={source} />
                            </div>
                        </div>
                    </div>
...
export const CaptionButton = ({ className, ...props }: React.ComponentProps<'button'>) => {
    return (
        <button
            className={cn(
                'px-2.5 size-8 [&_i]:text-sm! flex items-center justify-center gap-2 rounded-md hover:text-surface-900 dark:hover:text-surface-0 hover:bg-surface-200/50 dark:hover:bg-surface-800 text-surface-500 dark:text-surface-400 transition-colors',
                className
            )}
            {...props}
        />
