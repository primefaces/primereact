...
import { Tooltip, TooltipArrow, TooltipContent, TooltipPortal, TooltipPositioner, TooltipTrigger } from './tooltip';
...
export default function Playground() {
    return (
        <div className="w-full h-full overflow-auto">
            <div className="min-h-screen min-w-[200vw] flex items-start gap-24">
                <div className=" flex flex-col gap-72 py-60 items-start justify-center">
                    {items.map(({ side, align }) => (
                        <Tooltip key={side + align} defaultOpen={true}>
                            <TooltipTrigger className="ml-96 mt-20">Show Tooltip</TooltipTrigger>
                            <TooltipPortal>
                                <TooltipPositioner side={side} align={align} sideOffset={8}>
                                    <TooltipContent className="max-w-60 p-3">
                                        <span className="font-semibold text-surface-0 dark:text-surface-950 text-sm block">Quick Tip</span>
                                        <span className="text-surface-0 dark:text-surface-950 opacity-60 text-sm block mt-1">You can use tooltips to display helpful information on hover.</span>
                                        <TooltipArrow />
                                    </TooltipContent>
                                </TooltipPositioner>
                            </TooltipPortal>
