---
title: IconField API
description: API documentation for IconField component
component: iconfield
---

## IconFieldRoot

### Props

<DocTable name="IconFieldRoot" category="api" type="props" />

### Interfaces

<DocTable name="IconFieldRoot" category="api" type="interfaces" />

### Types

<DocTable name="IconFieldRoot" category="api" type="types" />

## InputIcon

### Props

<DocTable name="InputIcon" category="api" type="props" />

### Exposes

<DocTable name="InputIcon" category="api" type="exposes" />

### Interfaces

<DocTable name="InputIcon" category="api" type="interfaces" />

### Types

<DocTable name="InputIcon" category="api" type="types" />
