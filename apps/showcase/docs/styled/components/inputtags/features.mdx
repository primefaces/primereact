...
InputTags can be combined with typeahead functionality to provide suggestions as users type. Define an `options` array along with `optionLabel` to specify the display field. For grouped suggestions, use `optionGroupLabel` and `optionGroupChildren` properties. The `onComplete` callback is triggered on input change, allowing you to filter and update suggestions dynamically.
