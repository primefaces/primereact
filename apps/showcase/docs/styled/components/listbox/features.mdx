...
<DocDemoViewer name="listbox:basic-demo" mode="collapsible" />
...
Checkboxes can be displayed alongside each option by enabling the `checkbox` property. This works with `multiple` selection mode to provide a familiar multi-select UI. A `Listbox.Header` can also be added to display a title or additional controls.
