---
title: 'Divider'
description: 'Divider is used to separate contents.'
component: 'divider'
---

## Usage

```tsx
import { Divider } from 'primereact/divider';
```

```tsx
<Divider />
```

## Examples

### Basic

Divider is basically placed between the items to separate.

<DocComponentViewer name="divider:basic-demo" />

### Type

Style of the border is configured with the `type` property that can either be `solid`, `dotted` or `dashed`.

<DocComponentViewer name="divider:type-demo" />

### Vertical

Vertical divider is enabled by setting the `layout` property as `vertical`.

<DocComponentViewer name="divider:vertical-demo" />

### Content

Children are rendered within the boundaries of the divider where location of the content is configured with the `align` property. In horizontal layout, alignment options are `left`, `center` and`right` whereas vertical
mode supports `top`, `center` and `bottom`.

<DocComponentViewer name="divider:content-demo" />

### Login

Sample implementation of a login form using a divider with content.

<DocComponentViewer name="divider:login-demo" />

## Accessibility

### Screen Reader

Divider uses a `separator` role with `aria-orientation` set to either "horizontal" or "vertical".

### Keyboard Support

Component does not include any interactive elements.
