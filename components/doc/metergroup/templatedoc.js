...
export default function TemplateDemo() {
    ...
    const labelListRenderer = ({values}) => <div className='flex flex-wrap gap-3'>

    {values.map((item, index) => (<Card className="flex-1" key={index} pt={{ content: { className: 'p-0' }}}>
    ...
    const start = ({totalPercent}) =>  <div className="flex justify-content-between mt-3 mb-2 relative">
                    <span>Storage</span>
                    <span style={{ width: totalPercent + '%' }} className="absolute text-right">{totalPercent}%</span>
                    <span className="font-medium">1TB</span>
                </div>

    const end = <div className="flex justify-content-between mt-3">
