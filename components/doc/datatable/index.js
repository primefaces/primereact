...
`}
                    ...
                    <p>
                        DataTable requires a value as an array of objects and columns defined with Column component. Throughout the samples, a product interface having code, name, description, image, category, quantity, price, inventoryStatus and
                        rating properties is used to define an object to be displayed by the datatable. Products are loaded by a CustomerService that connects to a server to fetch the products. Note that this is only for demo purposes, DataTable does
...
`}
                    ...
                    <h5>Table Layout</h5>
                    <p>
                        Default table-layout is fixed meaning the cell widths do not depend on their content. If you require cells to scale based on their contents set <i>autoLayout</i> property to true. Note that Scrollable and/or Resizable tables
                    ...
                    <p>
                        ...
                        <i>header</i> and <i>footer</i> properties of a column are used to define the content of these sections by accepting either simple string values or JSX for advanced content. Similarly DataTable itself also provides{' '}
...
`}
                    ...
                    <p>
                        Getting access to the sorted data is provided by the <i>onValueChange</i> callback.
                    </p>
                    <CodeHighlight>
                        {`
...
`}
                    ...
                    <h5>Filtering</h5>
                    <p>
                        DataTable has advanced filtering capabilities that does the heavy lifting while providing flexible customization. Filtering has two layout alternatives defined with the <i>filterDisplay</i>. In <b>row</b> setting, filter
                        elements are displayed in a separate row at the header section whereas in <i>menu</i> mode filter elements are displayed inside an overlay. The template filter gets a <i>value</i>, <i>filterCallback</i> and{' '}
...
`}
                    ...
                    <p>
                        Getting access to the filtered data is provided by the <i>onValueChange</i> callback.
                    </p>
                    <CodeHighlight>
                        {`
...
`}
                    ...
                    <h5>Selection</h5>
                    <p>
                        DataTable provides single and multiple selection modes on click of a row or cell. Selected rows are bound to the <i>selection</i> property for reading and updated using <i>onSelectionChange</i> callback. Alternatively column
                        based selection can be done using radio buttons or checkboxes using <i>selectionMode</i> of a particular column. In addition <i>onRowSelect</i>-<i>onRowUnselect</i> / <i>onCellSelect</i>-<i>onCellUnselect</i> events are
                        provided as optional callbacks.
                    </p>
...
`}
                    ...
                    <p>
                        In multiple mode, selection binding should be an array and multiple items can either be selected using metaKey or toggled individually depending on the value of metaKeySelection property value which is true by default. On
                        touch enabled devices metaKeySelection is turned off automatically. Also, ShiftKey is supported for range selection. In addition, the rectangular selection can be dragged over the desired rows or cells thanks to the
...
`}
                    ...
                    <p>
                        If you prefer a radiobutton or a checkbox instead of a row click, use the <i>selectionMode</i> of a column instead. Following datatable displays a checkbox at the first column of each row and automatically adds a header
                        checkbox to toggle selection of all rows.
                    </p>
                    <p>
                        Tip: Use <i>showSelectionElement</i> function in case you need to hide selection element for a particular row.
                    </p>
                    <CodeHighlight>
                        {`
...
`}
                    ...
                    <h5>Cell Editing</h5>
                    <p>
                        Incell editing feature provides a way to quickly edit data inside the table. A cell editor is defined using the <i>editor</i> property that refers to a function to return an input element for the editing.
                    </p>
...
`}
                    ...
                    <p>
                        Clicking outside the cell or hitting enter key closes the cell, however this may not be desirable if the input is invalid. In order to decide whether to keep the cell open or not, provide a <i>cellEditValidator</i> function
                        that validates the value. Optionally <i>onCellEditComplete</i> and <i>onCellEditCancel</i>
                        events are available at the column component to provide callbacks whenever an editor is submitted or cancelled.
                    </p>
...
`}
                    ...
                    <p>
                        Row expansion allows displaying detailed content for a particular row. To use this feature, add an <i>expander</i> column, define a <i>rowExpansionTemplate</i> as a function to return the expanded content and bind to
...
`}
                    ...
                    <p>
                        You can choose which columns are <i>resizeable</i> per column.
                    </p>
                    <CodeHighlight>
                        {`
<DataTable value={products} resizableColumns>
...
`}
                    ...
                    <p>
                        Columns can be reordered using drag drop by setting the <i>reorderableColumns</i> to true. <i>onColReorder</i> is a callback that is invoked when a column is reordered. DataTable keeps the column order state internally using
...
`}
                    ...
                    <h5>Row Reorder</h5>
                    <p>
                        Data can be reordered using drag drop by adding a reorder column that will display an icon as a drag handle. <i>onRowReorder</i> is a callback that is invoked when a column is reordered, use this callback to update the new
                        order. The reorder icon can be customized using <i>rowReorderIcon</i> of the column component.
                    </p>
                    <p>
                        Tip: Use <i>showRowReorderElement</i> function in case you need to hide selection element for a particular row.
                    </p>
...
`}
                    ...
                    <h5>RowGrouping</h5>
                    <p>
                        RowGrouping has two modes defined be the <i>rowGroupMode</i> property, in "subheader" option rows are grouped by a groupRowsBy and in "rowspan" mode grouping is done based on the sort field. In both cases, data should be
                        sorted initally using the properties such as sortField and sortOrder. In "subheader" mode,
...
`}
                    ...
                    <h5>Scrolling</h5>
                    <p>
                        DataTable supports both horizontal and vertical scrolling as well as frozen columns and rows. Scrollable DataTable is enabled using <i>scrollable</i> property and <i>scrollHeight</i> to define the viewport height.
                    </p>
                    <CodeHighlight>
                        {`
...
`}
                    ...
                    <ul>
                        <li>
                            Use <i>min-width</i> in vertical scrolling only so that when there is enough space columns may grow and for smaller screens a horizontal scrollbar is displayed to provide responsive design.
                        </li>
                        <li>
                            When horizontal scrolling is enabled, prefer <i>width</i> instead of <i>min-width</i>.
                        </li>
                        <li>
                            In vertical scrolling only, use <i>flex</i> to disable grow and shrink while defining a initial width. When horizontal scrolling is enabled, this is not required as columns do not grow or shrink in horizontal scrolling.
                        </li>
                    </ul>
...
`}
                    ...
                    <h6>Flex Scroll</h6>
                    <p>
                        In cases where viewport should adjust itself according to the table parent's height instead of a fixed viewport height, set scrollHeight option as flex. In example below, table is inside a Dialog where viewport size
...
`}
                    ...
                    <p>
                        For horizontal scrolling, it is required to set <i>scrollDirection</i> to "horizontal" and give fixed widths to columns.
                    </p>
                    <CodeHighlight>
                        {`
...
`}
                    ...
                    <p>
                        Set <i>scrollDirection</i> to "both" and give fixed widths to columns to scroll both ways.
                    </p>
                    <CodeHighlight>
                        {`
...
`}
                    ...
                    <h6>Frozen Rows</h6>
                    <p>
                        Frozen rows are used to fix certain rows while scrolling, this data is defined with the <i>frozenValue</i> property.
                    </p>
...
`}
                    ...
                    <p>
                        Certain columns can be frozen by using the <i>frozen</i> property of the column component. In addition <i>alignFrozen</i> is available to define whether the column should be fixed on the left or right.
                    </p>
...
`}
                    ...
                    <p>
                        ...
                        easy as defining a unique <i>stateKey</i>, the storage to keep the state is defined with the <i>stateStorage</i> property that accepts session for sessionStorage and local for localStorage. Also, a special storage
                        implementation can be made with <i>customSaveState</i> and <i>customRestoreState</i> methods using <i>stateStorage="custom"</i>. Currently following features are supported by TableState; paging, sorting, filtering, column
...
`}
                    ...
                    <h5>Responsive</h5>
                    <p>
                        DataTable responsive layout can be achieved in two ways; first approach is displaying a horizontal scrollbar for smaller screens and second one is defining a breakpoint to display the cells of a row as stacked. Scrollable
...
`}
                    ...
                    <DevelopmentSection>
                        <h6>Screen Reader</h6>
                        <p>
                            DataTable uses a <i>table</i> element whose attributes can be extended with the <i>tableProps</i> option. This property allows passing aria roles and attributes like <i>aria-label</i> and <i>aria-describedby</i> to define
                        ...
                        <p>
                            The filter menu button use <i>aria.showFilterMenu</i> and <i>aria.hideFilterMenu</i> properties as <i>aria-label</i> in addition to the <i>aria-haspopup</i>, <i>aria-expanded</i> and <i>aria-controls</i> to define the
                            relation between the button and the overlay. Popop menu has <i>dialog</i> role with <i>aria-modal</i>
                            ...
                            <i>aria.clear</i> and <i>aria.apply</i> properties. <i>filterInputProps</i> of the Column component can be used to define aria labels for the built-in filter components, if a custom component is used with templating you
                            also may define your own aria labels as well.
                        </p>

                        <p>
                            Editable cells use custom templating so you need to manage aria roles and attributes manually if required. The row editor controls are button elements with <i>aria.editRow</i>, <i>aria.cancelEdit</i> and{' '}
