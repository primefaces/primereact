...
export class LiveEditor extends Component {
    ...
    renderCodeSandboxButton(nameWithExt, index, label, files, extDependencies) {
        ...
        return (
            <form key={index} action="https://codesandbox.io/api/v1/sandboxes/define" method="POST" target="_blank">
                ...
                <button type="submit" className="live-editor-button">
                    ...
                    <span className="live-editor-text">{label}</span>
                </button>
            </form>
        );
    }
...
ReactDOM.render(<${name} />, rootElement);`
            ...
            let label = 'Edit in CodeSandbox' + (key === 'ts' ? '(TS)' : (key === 'hooks' ? '(Hooks)' : ''));
            return this.renderCodeSandboxButton(`${name}${extension}`, index, label, _files, extDependencies);
        });
    }
